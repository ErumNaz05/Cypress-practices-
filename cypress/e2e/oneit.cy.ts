...
describe ('Sign in with email', ()=> {
    ...
    it("run fiter",()=>{
       ...
       //   cy.get('[class="_1c87gyz"]').click({force: true}) // select june
        ...
        //cy.get('button[class="c1tureqs dir dir-ltr"]')
        ...
        // cy.get('._1ku51f04').click()
     // it('view detail page',()=>{
    

        cy.log("view details")
            //cy.get('[class="o47luuh o1q97y5m dir dir-ltr"] > [class="m1dum4mk dir dir-ltr"] > buton[class="c1hv59mj dir dir-ltr"]').click()
            // cy.xpath('.//div[@class="m1dum4mk dir dir-ltr"]'[1])
            // .within(() => {
            //  cy.get('button[aria-label="Next photo: Townhouse in El Masnou"]').hover().click() //span[@class="iz6qhuf dir dir-ltr"]//*[name()="svg"]').click()
           ...
           cy.get('[class="_1e5q4qoz"]').then(($btn)=>{
                if($btn){  // not clicked
                    cy.get('[data-testid="pdp-save-button-unsaved"]').click({force: true})
                    cy.get('div[class="_rii3jc"]').type('wish1')
